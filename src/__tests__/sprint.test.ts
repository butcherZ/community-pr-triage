...
const makeScoredPR = (overrides: Omit<Partial<ScoredPR>, 'pr'> & { pr?: Partial<GitHubPR> } = {}): ScoredPR => {
  ...
  return {
    pr: {
      ...
      additions: 10, deletions: 5, changedFiles: 1, createdAt: '2026-01-01T00:00:00Z', updatedAt: '2026-02-15T00:00:00Z',
...
describe('selectSprintPRs', () => {
  it('selects up to count PRs', () => {
    const prs = Array.from({ length: 20 }, (_, i) =>
      makeScoredPR({ pr: { number: i + 1 }, value: { base: 30, severity: 0, status: 0, engagement: 0, urgency: 1.0, total: 50 - i } })
    );
  ...
  it('prioritizes urgent/high PRs', () => {
    const prs = [
      makeScoredPR({ pr: { number: 1 }, priority: 'low', value: { base: 30, severity: 0, status: 0, engagement: 0, urgency: 1.0, total: 100 } }),
      makeScoredPR({ pr: { number: 2 }, priority: 'urgent', value: { base: 30, severity: 0, status: 0, engagement: 0, urgency: 1.0, total: 50 } }),
      makeScoredPR({ pr: { number: 3 }, priority: 'high', value: { base: 30, severity: 0, status: 0, engagement: 0, urgency: 1.0, total: 40 } }),
    ];
  ...
  it('includes quick wins', () => {
    const prs = [
      makeScoredPR({ pr: { number: 1 }, priority: 'urgent', value: { base: 30, severity: 0, status: 0, engagement: 0, urgency: 1.0, total: 100 } }),
      makeScoredPR({ pr: { number: 2 }, priority: 'low', isQuickWin: true, value: { base: 30, severity: 0, status: 0, engagement: 0, urgency: 1.0, total: 30 } }),
    ];
    const result = selectSprintPRs(prs, 10);
  ...
  it('includes enhancements/features', () => {
    const prs = [
      makeScoredPR({ pr: { number: 1 }, priority: 'urgent', value: { base: 30, severity: 0, status: 0, engagement: 0, urgency: 1.0, total: 100 } }),
      makeScoredPR({ pr: { number: 2 }, prType: 'feature', priority: 'low', value: { base: 20, severity: 0, status: 0, engagement: 0, urgency: 1.0, total: 20 } }),
    ];
    const result = selectSprintPRs(prs, 10);
    expect(result.some((p) => p.pr.number === 2)).toBe(true);
  });
  ...
  it('does not duplicate PRs', () => {
    const prs = [
      makeScoredPR({ pr: { number: 1 }, priority: 'urgent', isQuickWin: true, prType: 'feature', value: { base: 30, severity: 0, status: 0, engagement: 0, urgency: 1.0, total: 100 } }),
    ];
    const result = selectSprintPRs(prs, 10);
...
describe('formatSprintUpdate', () => {
  ...
  it('includes Linear issue URLs when provided', () => {
    const prs = [
      makeScoredPR({ area: 'admin', pr: { number: 123 } }),
    ];
  ...
  it('omits Linear URL when not available for a PR', () => {
    const prs = [
      makeScoredPR({ area: 'admin', pr: { number: 456 } }),
    ];
